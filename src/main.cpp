...
// Copyright (c) 2013 PI developers
...
static const int64 nTargetTimespan = 188.4 * 10; // 31.4 minutes
static const int64 nTargetSpacing = 188.4; // 3.14 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 10 blocks

static const int64 nAveragingInterval = nInterval * 1; // 10 blocks
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PIMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PIMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running PIMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
